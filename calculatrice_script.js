...
$.fn.calculator = function (options) {
	...
	$(this).click(function () {
		...
		if (!settings.Is_resize) return this;

		$(".calcu_container ").resizable({
			ghost: true
		});

		// console.log("cloick = "+$.calculator.regional["a"].errorText);
	 	return this;
	});
	...
	/************draggable 2*************** */
	$("body").on("mousedown", ".myChampsCalc", function (downEvt) {
		//$(.titre modale) 
		...
		$draggable.closest('.calcu_container').one('bs.modal.hide', function () {
			$("body").off('mousemove.draggable');
		});
	...
	//
	$("body").on("mouseenter", ".myChampsCalc", function () {

		if (!settings.Is_draggable) return;
		$(".myChampsCalc").css("cursor", "move");
	});
  ...
  console.log($.calculator);
	...
	function total() {
		...
		//  try { 
			...
			// console.log($.calculator,options.langue);
			 
		// console.log("opt 0 ="+options.langue.trim());  
		var msg_err =$.calculator.regional[options.langue]?$.calculator.regional[options.langue].errorText:$.calculator.regional['en'].errorText; 
